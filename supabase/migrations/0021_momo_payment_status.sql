-- All Vegan — thêm trạng thái thanh toán để theo dõi đơn MoMo (chờ xác nhận /
-- đã thanh toán / thất bại). Đơn tiền mặt coi như "paid" ngay (thu tiền khi giao).
-- Chạy SAU 0020_payment_method.sql.
...
begin
  ...
  v_payment_status := case when p_payment_method = 'cash' then 'paid' else 'pending' end;
  ...
  insert into orders (
    ...
    store_lat, store_lng, delivery_lat, delivery_lng, estimated_minutes, payment_method, payment_status
  )
  values (
    v_order_id, auth.uid(), v_total, v_status, p_type, p_address, p_table_number, v_points, p_voucher_code,
    p_store_lat, p_store_lng, p_delivery_lat, p_delivery_lng, p_estimated_minutes, p_payment_method, v_payment_status
  )
