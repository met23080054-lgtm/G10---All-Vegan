-- All Vegan — thêm dữ liệu toạ độ để mô phỏng hành trình giao hàng trên bản đồ.
-- Chạy SAU 0004_seed.sql (vào project Supabase đã chạy 0001-0004 trước đó).
...
-- place_order cần ký hiệu mới (drop rồi tạo lại, vì CREATE OR REPLACE không cho thêm tham số
-- mới vào giữa danh sách tham số cũ một cách an toàn).
drop function if exists place_order(jsonb, text, text, text, text);
